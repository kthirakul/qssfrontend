import useAllDocs from './all-docs.use'

const AllDocsUi: React.FC = () => {
  const hook = useAllDocs()
  return hook.allDocs ? (
    <div className=' w-full h-full relative'>
      <div className=' text-2xl font-semibold text-center mt-12'>รายการคำขอทั้งหมด</div>
      <div className='p-12 flex flex-col gap-y-6'>
        {hook.allDocs.map((res) => {
          return res.docData.map((res1, index) => {
            return (

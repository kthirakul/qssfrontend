import useApprovedList from './approved-list.use'

const ApprovedListUi: React.FC = () => {
  const hook = useApprovedList()
  return hook.allDocs ? (
    <div className=' w-full h-full relative'>
      <div className=' text-2xl font-semibold text-center mt-12'>รายการคำขอที่ผ่านการคัดกรอง</div>
      <div className='p-12 flex flex-col gap-y-6'>
        {hook.allDocs.map((res) => {
          return res.docData
            .filter((res2) =>
              res2.statusLog.some((res) => res.status === 'คัดกรองคำขอตรวจสอบวุฒิแล้ว')
            )
            .filter(
              (res3) =>
                !res3.statusLog.some(
